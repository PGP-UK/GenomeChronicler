...
if(!@ARGV) {
	die "Please provide a single base recalibrated gVCF file to this script\n";
}
...
#------------------------------------------
#-- Convert gVCF to a gvcf that contains only
...
sub _gvcf_to_plink() {
	...
	#- the vcf has the SN (sample name) from the gVCF, so when converts that to plink it gives me problems if there is a "_" in the name. So I'm changin it
